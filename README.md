...
prism's history; on the alternate screen, each wheel tick is sent as three Up or Down keys instead. `[no-tty]` views
always keep mouse input local. Motion and drag events remain local to prism. The optional one-line debug bar appears at
the bottom and reports the most recently detected Crossterm key or mouse event, including its modifiers, event kind,
coordinates, and state where applicable.
